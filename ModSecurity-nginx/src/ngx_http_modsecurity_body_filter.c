...
//// wac_begin ////
            ngx_http_modsecurity_conf_t *mcf_2;
            mcf_2 = ngx_http_get_module_loc_conf(r, ngx_http_modsecurity_module);
            if(mcf_2->wacEnable==WAC_ON || mcf_2->wacEnable==WAC_STUDYONLY){
                WAC* pWAC;
                pWAC=ctx->pWAC;
                if(pWAC!=NULL){
                    //wac_study(pWAC,mcf_2->blackLevel);
