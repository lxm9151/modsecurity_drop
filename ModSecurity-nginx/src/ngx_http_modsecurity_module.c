...
//// wac_begin ////
    //ctx->pWAC=wac_create(ctx->modsec_transaction);
//// wac_end ////
...
//// wac_begin ////
char *
ngx_conf_set_wac_mem_addr(ngx_conf_t *cf, ngx_command_t *cmd, void *conf){
    ngx_str_t                         *value;
    const char                        *l_redis_ip, *l_redis_port;
    ngx_http_modsecurity_main_conf_t  *mmcf;

    value = cf->args->elts;
    l_redis_ip = ngx_str_to_char(value[1], cf->pool);
    l_redis_port = ngx_str_to_char(value[2], cf->pool);


    if (l_redis_ip == (char *)-1) {
        return NGX_CONF_ERROR;
    }

    if (l_redis_port == (char *)-1) {
        return NGX_CONF_ERROR;
    }

    mmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_modsecurity_module);

    ngx_str_set(&(mmcf->redisHost),l_redis_ip);
    mmcf->redisPort=atoi(l_redis_port);
    return NGX_CONF_OK;
}

char *
ngx_conf_set_wac(ngx_conf_t *cf, ngx_command_t *cmd, void *conf){
    ngx_str_t                         *value;
    const char                        *wac_enable;
    ngx_http_modsecurity_conf_t       *mcf = conf;

    value = cf->args->elts;
    wac_enable = ngx_str_to_char(value[1], cf->pool);

    if(strcasecmp(wac_enable,"on")==0){
        mcf->wacEnable=WAC_ON;
    }
    else if(strcasecmp(wac_enable,"studyOnly")==0){
        mcf->wacEnable=WAC_STUDYONLY;
    }
    else if(strcasecmp(wac_enable,"detectOnly")==0){
        mcf->wacEnable=WAC_DETECTONLY;
    }
    else{
        mcf->wacEnable=WAC_OFF;
    }
...
/****
char *
ngx_conf_set_wac_white_black(ngx_conf_t *cf, ngx_command_t *cmd, void *conf){
    ngx_str_t                         *value;
    ngx_http_modsecurity_conf_t       *mcf = conf;
    value = cf->args->elts;
    mcf->whiteLevel = atoi(ngx_str_to_char(value[1], cf->pool));
    mcf->blackLevel = atoi(ngx_str_to_char(value[2], cf->pool));
...
static ngx_command_t ngx_http_modsecurity_commands[] =  {
  ...
  //// wac_begin ////
  {
    ngx_string("wac_mem_addr"),
    NGX_HTTP_LOC_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_MAIN_CONF|NGX_CONF_TAKE2,
    ngx_conf_set_wac_mem_addr,
    NGX_HTTP_LOC_CONF_OFFSET,
    0,
    NULL
  },
  ...
  /*****
  {
    ngx_string("wac_white_black"),
    NGX_HTTP_LOC_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_MAIN_CONF|NGX_CONF_TAKE2,
    ngx_conf_set_wac_white_black,
    NGX_HTTP_LOC_CONF_OFFSET,
    0,
    NULL
  },
...
ngx_module_t ngx_http_modsecurity_module = {
    ...
    //// wac_begin ////
    //NULL,                                  /* init process */
    ngx_http_modsecurity_init_process,
    //// wac_end ////
    NULL,                                  /* init thread */
    NULL,                                  /* exit thread */
    //// wac_begin ////
    //NULL,                                  /* exit process */
    ngx_http_modsecurity_exit_process,
    //// wac_end ////
    NULL,                                  /* exit master */
...
#endif
//// wac_begin ////
    conf->wacEnable=NGX_CONF_UNSET;
//    conf->whiteLevel=NGX_CONF_UNSET;
//    conf->blackLevel=NGX_CONF_UNSET;
//// wac_end ////
...
//// wac_begin ////
    ngx_conf_merge_value(c->wacEnable, p->wacEnable, WAC_OFF);
//    ngx_conf_merge_value(c->whiteLevel, p->whiteLevel, -1);
//    ngx_conf_merge_value(c->blackLevel, p->blackLevel, 99);
//// wac_end ////
...
static
ngx_int_t ngx_http_modsecurity_init_process(ngx_cycle_t *cycle){
    
    ngx_http_modsecurity_main_conf_t  *mmcf;
    mmcf= ngx_http_cycle_get_module_main_conf(cycle, ngx_http_modsecurity_module)  ;
    if(mmcf->redisPort!=NGX_CONF_UNSET ){ 
        redisConn = redisConnect((char*)mmcf->redisHost.data,mmcf->redisPort); 
        if (!redisConn) {
            fprintf(stderr,"connect to redis fail\n");
            return NGX_ERROR;
        }
    }
    else{
...
static
void ngx_http_modsecurity_exit_process(ngx_cycle_t *cycle){
