...
//// wac_begin ////
        ...
        int con;
        con=msc_parse_request_body(ctx->modsec_transaction);
        if(!con){

            /// WAC检测 如果风险等级为1，直接跳过后面的filter
            if(mcf->wacEnable!=WAC_OFF){

                 WAC* pWAC;
                pWAC=wac_create(ctx->modsec_transaction);
                ctx->pWAC=pWAC;

                if(mcf->wacEnable==WAC_ON||mcf->wacEnable==WAC_DETECTONLY){
                    fprintf(stderr,"--------- WAC detect begin ----------\n");
                    //if(wac_detect(pWAC,mcf->whiteLevel,mcf->blackLevel)){
                    if(wac_detect(pWAC)){
                        fprintf(stderr,"--------- WAC 检测不通过! ----------\n\n");
                    }
                    else{
                        fprintf(stderr,"--------- WAC 检测通过! ----------\n\n");
                    }
                }
            }
            msc_eval_request_body(ctx->modsec_transaction);
        }
