...
int Transaction::processRequestBody() {
    ...
    return evalRequestBody();
}

//int Transaction::processRequestBody() {
int Transaction::parseRequestBody() {
/*** wac_end ***/
...
int Transaction::evalRequestBody() {
    /**** wac_end ****/
...
std::string Transaction::toOldAuditLogFormat(int parts,
    ...
    for (auto &h : l) {
        ...
        if(strcmp(h->getKeyWithCollection().c_str()+pos,"-src-ip-")==0){
            src_ip.assign(h->getValue());
        }
        else if(strcmp(h->getKeyWithCollection().c_str()+pos,"-src-port-")==0){
            src_port.assign(h->getValue());
        }
        if(strcmp(h->getKeyWithCollection().c_str()+pos,"-dst-ip-")==0){
            dst_ip.assign(h->getValue());
        }
        else if(strcmp(h->getKeyWithCollection().c_str()+pos,"-dst-port-")==0){
            dst_port.assign(h->getValue());
        }
    ...
    for (auto a : m_rulesMessages) {
        audit_log << tstr;
        audit_log << " [transaction_info " << this->m_id.c_str();
        if(src_ip==""){
...
/*** wac_begin ***/
extern "C" int msc_parse_request_body(Transaction *transaction) {
    return transaction->parseRequestBody();
}
extern "C" int msc_eval_request_body(Transaction *transaction) {
    return transaction->evalRequestBody();
}
/*** wac_end ***/
